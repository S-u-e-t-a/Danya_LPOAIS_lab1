...
void SplitText(const vector<string>& text, vector<string>& wordsWithSearchSymbol) {
  ...
  for (int i = 0; i < text.size(); i++) {
    ...
    while (text[i][j] != '\0') {
      ...
      //if (!binary_search(separators.begin(), separators.end(), text[i][j]) && (text[i][j] != string::npos)) { // мб мен€ть begin() или find вместо
      //  word.push_back(text[i][j]);
      //}
...
void FindSymbolInText(vector<string>& wordsWithSearchSymbol, string searchSymbol) {
  ...
  for (int i = 0; i < wordsWithSearchSymbol.size(); i++) {
    ...
    if (!(tempWord.find(searchSymbol) != string::npos)) { // если не нашЄл. убрать ! если хочу добавл€ть в новый вектор
...
//ѕодсчЄт количества символов, слов, строк, абзацев в тексте
//void Count_Amount_In_Text(const vector<string>& text) {
//  string letters = "qwetryuiopasdfghjklzxcbvnmйцукенгшщзхъфывапрлоджэ€чсмитьбюQWERTYUIOPASDFGHJKLZXCVBNM…÷” ≈Ќ√Ўў«’Џ‘џ¬јѕ–ќЋƒ∆Ёя„—ћ»“№Ѕё";
